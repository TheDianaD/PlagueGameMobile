(function(storyContent) {
    ...
    function continueStory(firstTime) {
        ...
        // ── GRAVE IMAGE IN STORY COLUMN ──────────────────────────────
        // If story has ended and player is dead, show grave image inside the story text
        if (!story.canContinue && story.currentChoices.length === 0 && window.sceneState.isDead) {
            var storyDiv = document.getElementById('story');
            if (storyDiv && !storyDiv.querySelector('.story-grave-img')) {
                var graveWrap = document.createElement('div');
                graveWrap.className = 'story-grave-img';
                graveWrap.style.cssText = 'margin: 2rem auto 1rem; max-width: 420px; width: 100%;';
                var graveImg = document.createElement('img');
                graveImg.src = 'grave.webp';
                graveImg.alt = 'A grave in the rain';
                graveImg.style.cssText = 'width:100%; border-radius:3px; opacity:0.92; display:block;';
                graveWrap.appendChild(graveImg);
                storyDiv.appendChild(graveWrap);
                scrollDown(storyDiv.scrollHeight);
            }
        }
    }
